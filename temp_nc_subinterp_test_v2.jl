...
lines!(getindex.(pathall,1),getindex.(pathall,2),color=error_all)
#plot(getindex.(pathall,1),getindex.(pathall,2),getindex.(pathall,3),lw=5)


Colorbar(f[1, 2], limits = (minimum(error_all), maximum(error_all)), colormap = :viridis,
    flipaxis = false)